...
const imageTargetPipelineModule = () => {
  const modelFile = `${process.env.PUBLIC_URL}/model/jellyfish-model.glb`
  const videoFile = `${process.env.PUBLIC_URL}/video/jellyfish-video.mp4`
  ...
  const showTarget = ({ detail }) => {
    ...
    // This string must match the name of the image target uploaded to 8th Wall.
    if (detail.name === 'video-target' || detail.name === 'book') {
